  ...
  return (
    <div className="h-full bg-gray-50 text-gray-900">
      ...
      <main className={page === 'login' || page === 'signup' || page === 'forgot_password' ? '' : 'pl-64 pt-16'}>
        {page === 'integrations' ? <Integrations /> : page === 'loans' ? <Loans /> : page === 'logs' ? <Logs /> : page === 'reports' ? <Reports /> : page === 'roles_permissions' ? <RolesPermissions /> : page === 'tenants' ? <Tenants /> : page === 'users' ? <Users /> : page === 'settings' ? <Settings /> : page === 'profile' ? <Profile /> : page === 'notifications' ? <Notifications /> : page === 'forgot_password' ? <ForgotPassword /> : page === 'signup' ? <Signup /> : page === 'login' ? <Login /> : <Dashboard />}
      </main>
    </div>
  )
}
