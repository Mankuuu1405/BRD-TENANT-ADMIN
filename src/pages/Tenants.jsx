...
export default function Tenants() {
  ...
  const [form, setForm] = useState({ name: '', interest_rate: 0, tenure_months: 0, max_amount: 0, processing_fee_pct: 0, status: 'Active', description: '', loan_type: 'personal' })
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(p => (p.name || '').toLowerCase().includes(q) || (p.type_of_loan || '').toLowerCase().includes(q))
  }, [search, products])
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="h-9 px-3 rounded-lg bg-primary-600 text-white" onClick={() => { setCreating(true); setForm({ name: '', interest_rate: 0, tenure_months: 0, max_amount: 0, processing_fee_pct: 0, status: 'Active', description: '', loan_type: 'personal' }) }}>+ Add Product</button>
      </div>
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} className="border-t border-gray-100">
                <td className="px-4 py-3 font-medium">{p.type_of_loan || p.name}</td>
                ...
                <td className="px-4 py-3 flex gap-2">
                  ...
                  {/* Edit/Delete features would need explicit backend endpoints */}
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 bg-black/25 grid place-items-center z-40">
          <div className="bg-white w-full max-w-lg rounded-xl border border-gray-200 shadow-card p-4">
            ...
            <div className="mt-3 space-y-3">
              ...
              <label className="block"><div className="text-sm text-gray-700">Loan Type</div>
                <select value={form.loan_type} onChange={(e)=>setForm({ ...form, loan_type: e.target.value })} className="mt-1 w-full h-9 rounded-lg border border-gray-300 px-3">
        ...
        <div className="fixed inset-0 bg-black/25 grid place-items-center z-40" onClick={()=>setViewing(null)}>
          <div className="bg-white w-full max-w-lg rounded-xl border border-gray-200 shadow-card p-4" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="mt-3 space-y-2 text-sm">
              <div className="font-medium">{viewing.type_of_loan}</div>
              <div className="text-gray-700">Type: {viewing.subcategory}</div>
